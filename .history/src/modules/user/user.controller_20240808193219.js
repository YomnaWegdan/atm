...
const signUP = catchError(async (req , res , next) =>{
    ...
    const user = await userModel.findOne({username:user.toLowerCase()})
    ...
    // const link = `${req.protocol}://${req.headers.hostname}/auth/verify/${token}`

    const refreshToken = jwt.sign({email} , process.env.signatureKeyRefresh , {expiresIn:60 })
    // const refreshLink = `${req.protocol}://${req.headers.hostname}/auth/refreshToken/${refreshToken}`
